...
#sudo aa-remove-unknown --> se non riesce a killare immagini
...
try:
    ...
    mysql_srv = mgr.addContainer(
        ...
        docker_args={
            "volumes": {
                prj_folder + "/database/oai_db2.sql": {
    ...
    udr_srv = mgr.addContainer(
        name = "udr_srv",
        dhost = "udr",
        dimage = "oaisoftwarealliance/oai-udr:v2.0.0",
        dcmd = "",
        docker_args={
            "volumes": {
                prj_folder + "/conf/basic_nrf_config.yaml": {
    ...
    udm_srv = mgr.addContainer(
        name = "udm_srv",
        dhost = "udm",
        dimage = "oaisoftwarealliance/oai-udm:v2.0.0",
        dcmd = "",
        docker_args={
            "volumes": {
                prj_folder + "/conf/basic_nrf_config.yaml": {
    ...
    ausf_srv = mgr.addContainer(
        name = "ausf_srv",
        dhost = "ausf",
        dimage = "oaisoftwarealliance/oai-ausf:v2.0.0",
        dcmd = "",
        docker_args={
            "volumes": {
                prj_folder + "/conf/basic_nrf_config.yaml": {
    ...
    nrf_srv = mgr.addContainer(
        name = "nrf_srv",
        dhost = "nrf",
        dimage = "oaisoftwarealliance/oai-nrf:v2.0.0",
        dcmd = "",
        docker_args={
            "volumes": {
                prj_folder + "/conf/basic_nrf_config.yaml": {
    ...
    amf_srv = mgr.addContainer(
        name = "amf_srv",
        dhost = "amf",
        dimage = "oaisoftwarealliance/oai-amf:v2.0.0",
        dcmd = "",
        docker_args={
            "volumes": {
                prj_folder + "/conf/basic_nrf_config.yaml": {
    ...
    smf_srv = mgr.addContainer(
        name = "smf_srv",
        dhost = "smf",
        dimage = "oaisoftwarealliance/oai-smf:v2.0.0",
        dcmd = "",
        docker_args={
            "volumes": {
                prj_folder + "/conf/basic_nrf_config.yaml": {
    ...
    upf_srv = mgr.addContainer(
        name="upf_srv",
        dhost = "upf",
        dimage="oaisoftwarealliance/oai-upf-vpp:v2.0.0",
        dcmd = "",
        docker_args={
            "environment": {
    ...
    ext_dn_srv = mgr.addContainer(
        ...
        dcmd = "",
        docker_args={
            "entrypoint": "/bin/bash -c \"iptables -t nat -A POSTROUTING -o ext_dn-upf -j MASQUERADE; ip route add 12.1.1.0/24 via 192.168.73.144 dev ext_dn-upf; ip route; sleep infinity\"",
    ...
    info("*** Adding ueransim container\n")
    ueransim_srv = mgr.addContainer(
        name = "gnb_srv",
        dhost = "gnb",
        dimage = "soldera21/ueransim:latest",
